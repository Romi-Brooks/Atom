...
- Music、SFX、Mixer、Transition 保持可独立组合，不强制引入统一 `AudioSystem`。
...
- [!] 后续版本实现。
- [ ] 让 Music Source 持有分块解码器，而不是在 Load 时保存完整 PCM。
- [ ] 引入固定容量 ring buffer/双缓冲、预读水位、EOF 与解码错误状态。
- [ ] SFX 继续使用全量 PCM，不与流式音乐强行统一。
