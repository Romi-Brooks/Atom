...
auto ToSampleFormat(const DecoderInfo& info) -> std::optional<AudioSampleFormat> {
    if (info.is_float && info.bits_per_sample == 32) return AudioSampleFormat::Float32;
    switch (info.bits_per_sample) {
        case 8: return AudioSampleFormat::Unsigned8;
        case 16: return AudioSampleFormat::Signed16;
        case 24:
        case 32: return AudioSampleFormat::Signed32;
...
auto Expand24To32(const std::vector<uint8_t>& packed) -> std::vector<uint8_t> {
    ...
    for (std::size_t src = 0, dst = 0; src + 2 < packed.size(); src += 3, dst += 4) {
        int32_t sample = static_cast<int32_t>(packed[src]) |
                         (static_cast<int32_t>(packed[src + 1]) << 8) |
...
auto AudioClipLoader::Load(const std::string& path) const -> std::optional<DecodedAudio> {
    ...
    if (!format || info.sample_rate == 0 || info.channels == 0) return std::nullopt;
    ...
    decoder->Close();
    if (pcm.empty()) return std::nullopt;
