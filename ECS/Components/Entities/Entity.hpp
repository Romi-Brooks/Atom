...
class Entity {
    protected:
        std::unique_ptr<render::ITexture> texture_;
        float radius_ = 0;
        render::Color color_{render::Color::White()};
        mutable algo::Vec2 position_; // mutable so const Move() can update it
    ...
    public:
        ...
        Entity(Entity&& other) noexcept
            : texture_(std::move(other.texture_)), radius_(other.radius_), color_(other.color_),
              position_(other.position_), hp_(other.hp_), attack_(other.attack_), move_speed_(other.move_speed_),
        ...
        auto CreateCircleWithColor(float radius, const render::Color& color) -> void {
            radius_ = radius;
            color_ = color;
        }
        ...
        [[nodiscard]] auto GetPosition() const -> algo::Vec2 {
            return position_;
        }

        virtual auto Draw(render::IRenderTarget& target) const -> void {
            if (texture_) {
