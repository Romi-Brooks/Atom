...
namespace atom::tools {
	...
	auto Packager::SafeRelativePath(const fs::path& path) -> std::string {
		try {
			...
			LOG_ERROR(atom::LogChannel::ATOM_UTILITIES_PACKAGER, "Unable to obtain relative path: " + std::to_string(*e.what()) );
			return path.filename().string();
		}
	}

	auto Packager::CollectFiles(const std::vector<std::string>& resourcePaths, std::vector<fs::path>& allFiles, const Config& config) -> bool {
		for (const auto& path_str : resourcePaths) {
			try {
				...
				LOG_WARNING(atom::LogChannel::ATOM_UTILITIES_PACKAGER, "Error occurred while processing the path " + path_str + ": " + e.what());
			}
		}
	...
	auto Packager::GenerateInternalFilename(const fs::path& filePath, const Config& config) -> std::string {
		try {
			...
			LOG_ERROR(atom::LogChannel::ATOM_UTILITIES_PACKAGER, "Failed to generate internal filename: " + std::to_string(*e.what()));
			return "unknown_file_" + std::to_string(file_index_.size());
		}
	}

	auto Packager::Pack(const std::vector<std::string>& resourcePaths, const std::string& outputFile, const Config& config) -> Result {
		file_table_.clear();
		file_index_.clear();
		...
		try {
			...
			LOG_ERROR(atom::LogChannel::ATOM_UTILITIES_PACKAGER, "Unable to delete existing file: " + std::to_string(*e.what()));
			return Result::ERROR_WRITE_FAILED;
		}
		...
		for (const auto& filePath : allFiles) {
			...
			try {
				...
				LOG_WARNING(atom::LogChannel::ATOM_UTILITIES_PACKAGER, "Warning: Unable to open file: " + SafePathToString(filePath)
						  + " - " + e.what());
				continue;
			}

			if (!input.is_open()) {
				LOG_WARNING(atom::LogChannel::ATOM_UTILITIES_PACKAGER, "Unable to open file: " + SafePathToString(filePath));
				continue;
			}
			...
			try {
				...
				LOG_ERROR(atom::LogChannel::ATOM_UTILITIES_PACKAGER, "Error: Failed to create file entry: " + std::to_string(*e.what()));
				input.close();
				continue;
			}
			...
			if (!input) {
				LOG_WARNING(atom::LogChannel::ATOM_UTILITIES_PACKAGER, "Failed to read file: " + SafePathToString(filePath));
				input.close();
				continue;
			}
			input.close();
			...
			LOG_INFO(atom::LogChannel::ATOM_UTILITIES_PACKAGER, "Packing: " + entry.filename + " (" + std::to_string(file_size) + " bytes)");
		}
		...
		if (verify.is_open()) {
			...
			LOG_INFO(atom::LogChannel::ATOM_UTILITIES_PACKAGER, "Verification version: " + std::to_string(version) + " " + (version == 1 ? "True" : "False"));

			uint32_t file_count_verify;
			verify.read(reinterpret_cast<char*>(&file_count_verify), sizeof(file_count_verify));
			LOG_INFO(atom::LogChannel::ATOM_UTILITIES_PACKAGER, "Number of documents to be verified: " + std::to_string(file_count_verify) + " " + (file_count_verify == file_count ? "True" : "False"));
			...
			LOG_INFO(atom::LogChannel::ATOM_UTILITIES_PACKAGER, "Verification file table offset: " + std::to_string(table_offset_verify) + " "
	...
	auto Packager::PrintPackageInfo() const -> void {
		LOG_INFO(atom::LogChannel::ATOM_UTILITIES_PACKAGER, "The package contains " + std::to_string(file_table_.size()) + " files:");
		for (const auto& entry : file_table_) {
			LOG_INFO(atom::LogChannel::ATOM_UTILITIES_PACKAGER, "  " + entry.filename + " [" + entry.type + "] - "
