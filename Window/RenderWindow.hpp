/**
  ...
  * @attention      : Wraps an IRenderWindow (created via RenderBackendRegistry)
...
class RenderWindow {
private:
    std::unique_ptr<IRenderWindow> window_;
    ...
    struct EventListenerEntry {
        uint64_t id;
        std::function<void(IEvent&)> fn;
        using ListenerFn = std::function<void(IEvent&)>;
    };
...
public:
    ...
    using RawEventListener = std::function<void(const void*)>;   // raw backend event, before translation (platform adapters only)
    using EventListener = std::function<void(IEvent&)>;          // translated engine event
    ...
    // registered in RenderBackendRegistry). Defaults to the engine default.
    auto Initialize(const std::string& title, Vec2 resolution,
                    std::string_view backendId = RenderBackendRegistry::kDefaultBackendId) -> void;
    auto Run() -> void;
