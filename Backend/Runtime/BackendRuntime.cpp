...
BackendRuntime::BackendRuntime() {
    ...
    if (!audio_backend_) throw std::runtime_error("Failed to initialize default SDL3 audio backend");
...
auto BackendRuntime::RegisterAvailableBackends() -> void {
    ...
    registry_.RegisterAudioDecoderBackend("sdl3", [](AudioDecoderRegistry& decoders) {
        return RegisterSDL3AudioDecoders(decoders);
    });
    registry_.RegisterAudioDecoderBackend("builtin", [](AudioDecoderRegistry& decoders) {
        return RegisterBuiltinAudioDecoders(decoders);
    });
}

auto BackendRuntime::Audio() -> IAudioBackend& {
    if (!audio_backend_) throw std::runtime_error("No active audio backend");
    return *audio_backend_;
}
auto BackendRuntime::AudioDecoders() -> AudioDecoderRegistry& { return audio_decoders_; }
auto BackendRuntime::Registry() -> BackendRegistry& { return registry_; }
...
auto BackendRuntime::NotifyAudioBackendChanging() -> void {
    ...
    for (auto* listener : listeners) if (listener) listener->OnAudioBackendChanging();
}

auto BackendRuntime::NotifyAudioDecoderBackendChanging() -> void {
    const auto listeners = audio_listeners_;
    for (auto* listener : listeners) if (listener) listener->OnAudioDecoderBackendChanging();
}
