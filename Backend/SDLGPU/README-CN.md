...
顶层 `Render/Pipeline` 保存后端无关的 `PipelineDesc`；本目录的 pipeline
代码负责将它转换为 `SDL_GPUGraphicsPipelineCreateInfo`。当前内置 Renderer2D
已经通过 `SDLGPU2DPipelineFactory` 集中创建 primitive/post-process 管线，避免
编码器重复管理 shader 生命周期。
