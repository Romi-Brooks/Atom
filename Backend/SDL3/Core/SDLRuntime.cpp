...
auto ToIndex(const SDLSubsystem subsystem) -> std::size_t {
    switch (subsystem) {
        case SDLSubsystem::Video: return 0;
        case SDLSubsystem::Audio: return 1;
        case SDLSubsystem::Events: return 2;
    }
    return 0;
}
...
auto ToSDLFlag(const SDLSubsystem subsystem) -> SDL_InitFlags {
    switch (subsystem) {
        case SDLSubsystem::Video: return SDL_INIT_VIDEO;
        case SDLSubsystem::Audio: return SDL_INIT_AUDIO;
        case SDLSubsystem::Events: return SDL_INIT_EVENTS;
    }
    return 0;
}
} // namespace

SDLSubsystemLease::SDLSubsystemLease(const SDLSubsystem subsystem) { Acquire(subsystem); }
SDLSubsystemLease::~SDLSubsystemLease() { Reset(); }
...
auto SDLSubsystemLease::Acquire(const SDLSubsystem subsystem) -> bool {
    ...
    if (g_subsystem_ref_counts[index] == 0 && !SDL_InitSubSystem(ToSDLFlag(subsystem))) return false;
...
auto SDLSubsystemLease::IsValid() const -> bool { return valid_; }
} // namespace atom
