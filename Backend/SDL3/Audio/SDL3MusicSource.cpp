...
auto SDL3MusicSource::EnsureStream() -> bool {
    ...
    LOG_DEBUG(atom::LogChannel::SDL_BACKEND_AUDIO, "Audio stream opened successfully");
    return true;
}
...
auto SDL3MusicSource::Play() -> void {
    if (pcm_data_.empty()) {
        LOG_WARNING(atom::LogChannel::ATOM_AUDIO_MUSIC, "Play() called with no data");
        return;
    }
    if (!EnsureStream()) {
    ...
    LOG_INFO(atom::LogChannel::ATOM_AUDIO_MUSIC, "Playback started");
}
...
auto SDL3MusicSource::DecodeLoop() -> void {
    ...
    while (thread_running_.load() && state_.load() == AudioSourceState::Playing) {
        ...
        if (cursor < pcm_data_.size()) {
            ...
            if (loop_.load()) {
                play_cursor_ = 0;
                LOG_DEBUG(atom::LogChannel::SDL_BACKEND_AUDIO, "Looping: rewound cursor to 0");
                continue;
            }
    ...
    thread_running_ = false;
    LOG_DEBUG(atom::LogChannel::SDL_BACKEND_AUDIO, "DecodeLoop exited");
}
