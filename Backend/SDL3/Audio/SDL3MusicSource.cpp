...
auto SDL3MusicSource::EnsureStream() -> bool {
    ...
    LOG_DEBUG(atom::backend::sdl::LogChannel::AUDIO, "Audio stream opened successfully");
    return true;
}
...
auto SDL3MusicSource::DecodeLoop() -> void {
    ...
    LOG_DEBUG(atom::backend::sdl::LogChannel::AUDIO, "DecodeLoop started: chunk=" + std::to_string(kStreamChunk) +
    ...
    while (thread_running_.load() && state_.load() == AudioSourceState::Playing) {
        ...
        if (cursor < pcm_data_.size()) {
            ...
            if (now - last_debug_log >= std::chrono::seconds(3)) {
                LOG_DEBUG(atom::backend::sdl::LogChannel::AUDIO, "Pushed " + std::to_string(toPush) +
            ...
            if (loop_.load()) {
                play_cursor_ = 0;
                LOG_DEBUG(atom::backend::sdl::LogChannel::AUDIO, "Looping: rewound cursor to 0");
                continue;
            }
    ...
    thread_running_ = false;
    LOG_DEBUG(atom::backend::sdl::LogChannel::AUDIO, "DecodeLoop exited");
}
