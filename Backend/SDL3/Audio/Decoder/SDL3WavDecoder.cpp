...
auto ReadU16LE(SDL_IOStream* io, std::uint16_t& value) -> bool {
    ...
    if (!ReadExact(io, bytes.data(), bytes.size())) return false;
...
SDL3WavDecoder::~SDL3WavDecoder() { Close(); }

auto SDL3WavDecoder::Open(const std::string& path) -> bool {
    Close();
    io_ = SDL_IOFromFile(path.c_str(), "rb");
    if (!io_) {
        LOG_ERROR(LogChannel::SDL_BACKEND_AUDIO,
                  "SDL_IOFromFile failed: " + std::string(SDL_GetError()));
        return false;
    }

    std::array<char, 4> id{};
    std::uint32_t riff_size = 0;
    if (!ReadExact(io_, id.data(), id.size()) || std::memcmp(id.data(), "RIFF", 4) != 0 ||
        !ReadU32LE(io_, riff_size) || !ReadExact(io_, id.data(), id.size()) ||
        std::memcmp(id.data(), "WAVE", 4) != 0) {
        Close();
        return false;
    }
    ...
    while (!have_data) {
        ...
        if (!ReadExact(io_, id.data(), id.size()) || !ReadU32LE(io_, chunk_size)) break;
        ...
        if (std::memcmp(id.data(), "fmt ", 4) == 0 && chunk_size >= 16) {
            ...
            std::uint16_t bits = 0;
            if (!ReadU16LE(io_, encoding) || !ReadU16LE(io_, channels) ||
                !ReadU32LE(io_, sample_rate) || !ReadU32LE(io_, byte_rate) ||
                !ReadU16LE(io_, block_align) || !ReadU16LE(io_, bits) ||
                (encoding != 1 && encoding != 3) || (encoding == 3 && bits != 32)) break;
        ...
        if (SDL_SeekIO(io_, next, SDL_IO_SEEK_SET) < 0) break;
    }

    const auto bytes_per_frame = static_cast<std::uint64_t>(info_.channels) *
                                 (source_bits_per_sample_ / 8u);
    if (!have_data || bytes_per_frame == 0 ||
        SDL_SeekIO(io_, data_offset_, SDL_IO_SEEK_SET) < 0) {
        Close();
        return false;
    }
...
auto SDL3WavDecoder::DecodeChunk(uint8_t* output, const uint32_t max_bytes) -> uint32_t {
    if (!io_ || !output || max_bytes == 0 || bytes_read_ >= data_size_) return 0;
    ...
    if (source_bits_per_sample_ == 24) {
        ...
        if (input_frame_bytes == 0 || max_bytes < output_frame_bytes) return 0;
        ...
        if (decoded == 0 && SDL_GetIOStatus(io_) == SDL_IO_STATUS_ERROR) {
            LOG_ERROR(LogChannel::SDL_BACKEND_AUDIO,
                      "WAV stream read failed: " + std::string(SDL_GetError()));
            Close();
        }
...
auto SDL3WavDecoder::Rewind() -> bool {
    if (!io_ || SDL_SeekIO(io_, data_offset_, SDL_IO_SEEK_SET) < 0) return false;
    bytes_read_ = 0;
    return true;
}

auto SDL3WavDecoder::GetInfo() const -> const DecoderInfo& { return info_; }
auto SDL3WavDecoder::IsOpen() const -> bool { return io_ != nullptr; }
