...
auto Minimp3Decoder::OpenFromMemory(const void* data, const std::size_t size) -> bool {
    ...
    if (!data || size == 0) {
        LOG_ERROR(LogChannel::ATOM_AUDIO_MINIMP3, "Minimp3: invalid in-memory buffer");
        return false;
    }
    ...
    const auto& frame_info = impl_->dec.info;
    if (!ValidateMp3Frame(frame_info)) {
        LOG_ERROR(LogChannel::ATOM_AUDIO_MINIMP3, "Minimp3: no valid MPEG audio frame found in memory buffer");
        Close();
        return false;
    }
    ...
    LOG_DEBUG(LogChannel::ATOM_AUDIO_MINIMP3,
