...
auto BackendRegistry::RegisterAudioDecoderBackend(std::string id, AudioDecoderInstaller installer) -> bool {
    if (!installer)
        return false;
...
auto BackendRegistry::InstallAudioDecoderBackend(const std::string_view id, AudioDecoderRegistry& registry) const
    -> bool {
